        ...
        if usage < usage_threashold:
            ...
            subprocess.run(
                [
                    ...
                    "-i", 
                    key_file,
                    f"../pyscf/test_set/graphene_mol_r_6_6_h_{task_ids[k]}.py",
                    f"{ec2_address}:./"
                ]
            )
            subprocess.run(
            ...
            ssh_command = f"""
                ...
                nohup tmux new -d 'python3 batch_run.py' \; pipe-pane 'cat > graphene_mol_r_6_6_h_{task_ids[k]}.log'
            """
                # tmux new -d 'python3 batch_run.py';
                # nohup tmux new -d 'mkdir abcd; echo q2421432' \; pipe-pane 'cat > graphene_mol_r_6_6_h_{task_ids[k]}.log'
            command = [
                'ssh',
                '-T',
                '-i',
