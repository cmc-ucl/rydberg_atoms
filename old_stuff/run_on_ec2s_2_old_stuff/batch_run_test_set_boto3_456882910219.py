...
# still need to run
# [202, 203, 204, 205, 206, 207, 208, 209, 210, 211, 212, 213, 214, 215, 216, 217, 218, 219, 220, 228, 241, 104, 106]
for region in regions:
    ...
    for ip, instance_id in zip(public_ips, ids):
        ...
        if usage < 5:
            try:
                subprocess.run(['sh', 'run_test_set_456882910219.sh', f"{region}", f"{ip}", f"{task_ids[k]}"], stdout=subprocess.DEVNULL)
